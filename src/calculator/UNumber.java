...
public class UNumber implements Comparable<UNumber> {
	...
	static Double[] estimation = new Double[]{0.0,1.0,1.4142135623730951,1.7320508075688772,2.0,2.23606797749979,
	...
	//private Object numSignificantDigits;
	...
	public UNumber(double v, int x){
		 ...
		 if (tempD[x+1]>=5){
			 ...
			 // left end (this tells up normalization is not required).
			 if (carry) {
				 for (int i=x+1; i>0; i-- ) tempD[i] = tempD[i-1];
				 tempD[0]=1;
				 this.dP++;
			 }
		 }
...
private int numSignificantDigits = 20;
	...
	public void sqrt() {
		UNumber newGuess = new UNumber(0, numSignificantDigits) ;
		...
		if ((getDouble()+"").length() > 0) {
			...
			// Does this input line consist of a value?
				...
				UNumber theValue =  new UNumber(inputValue, numSignificantDigits);
				
				UNumber two = new UNumber(2.0, numSignificantDigits);
				
				newGuess = estimation(theValue.toString().substring(1));			// Compute the estimate
				UNumber est =  newGuess;
				UNumber oldGuess = new UNumber(0, numSignificantDigits);										// Temporary value for determining when to terminate the loop				
				int iteration = 0;										// Count the number of iterations
				int digitsMatch = 0;
				System.out.println("iteration | oldGuess | newGuess | digitsMatch");
				do {
					...
					System.out.println("     " + iteration + " estimate " + (stop-start) + " nano seconds" );	
	...
	public UNumber estimation(String num) {
		System.out.println("num"+num);
		UNumber un = new UNumber(num);
		String[] nums = un.toString().split("e|E");
		String des = nums[0];
		System.out.println(un.getDouble());
		if (un.getDouble() == 0.0) {
			
			return un;
		}
		UNumber temp = new UNumber(des);
